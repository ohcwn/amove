...
const getImg=(()=>{const c=["nyaa","moe","desu","dere"];return a=>(a=a.shikimori_id)?`https://${c[a%c.length]}.shikimori.me/system/animes/original/${a}.jpg?${1723345562+ +a}`:"no-img.jpg"})(),parse=async c=>{const a=new Map;await Promise.all(c.map(async b=>{const d=await sha(b.worldart_link||b.shikimori_id||b.imdb_id||b.kinopoisk_id||`${b.type}${b.year}${b.title_orig}`);a.has(d)?a.get(d).top.last_episode<b.last_episode&&(a.get(d).top=b,a.get(d).ep=b.last_episode):a.set(d,{key:d,img:getImg(b),y:b.year,
ep:b.last_episode,top:b,raw:new Map,tr:new Map});a.get(d).raw.set(b.id,b);a.get(d).tr.set(b.translation.id,b.id)}));return a},buildString=c=>{const {top:a}=c;let b="";c.y&&(b+=`${c.y}\u0433. `);c.ep&&(b+=`${c.ep} \u044d\u043f. `);let d="";c.raw.forEach(e=>{""!==d&&(d+=", ");d=e===a?d+`<span class="underline">${e.translation.title}</span>`:d+e.translation.title;e.last_episode&&(d+=`[${e.last_episode}]`)});return b+d},buildHTML=c=>{let a="";c.forEach((b,d)=>{const {top:e}=b;let f,g,h;a+=`<div class="item" data-key="${d}">
<div class="left">
