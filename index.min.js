...
document.getElementById("new-button").addEventListener("click",async()=>{hash.clear();hash.set("new");const c=document.getElementById("list"),a=document.getElementById("status");c.style.display="none";var b=await sendQuery("list?types=anime,anime-serial&limit=100");const [d,e]=b;0!==d.length&&(b=await parse(d.reverse()),items=new Map([...b].reverse()),a.textContent=`\u041d\u0430\u0439\u0434\u0435\u043d\u043e: ${items.size} [${d.length}] (${e/1E3} \u0441\u0435\u043a.)`,c.innerHTML=buildHTML(items),
c.style.display="block")},{passive:!0});const searchForm=document.getElementById("search-form"),searchInput=document.getElementById("search-input");
...
`title=${encodeURI(d)}`;d=await sendQuery(`search?${d}`);var [f,g]=d;0!==f.length&&(items=await parse(f),a&&1<f.length&&([[,d]]=items,d.raw.has(a)&&(d.top=d.raw.get(a))),e.textContent=`\u041d\u0430\u0439\u0434\u0435\u043d\u043e: ${items.size} [${f.length}] (${g/1E3} \u0441\u0435\u043a.)`,c.innerHTML=buildHTML(items),c.style.display="block")}});
